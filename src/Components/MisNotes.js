...
export default function MisNotes() {
  ...
  useEffect(() => {
    axios
      .get(
        "https://inotes-backend-server-production.up.railway.app/api/notes/mis"
      )
      .then((response) => {
  ...
  return (
    <div className="container font-size-2x">
      <h1 className="center bold">Management Information System</h1>
      {error ? (
        <p>{error}</p>
      ) : (
        topics.map((topic) => (
